...
export default async function ResearchPost({params}:{params:Promise<{slug:string}>}){
  ...
  const publicationDate=report?AUG17_RESEARCH_PUBLICATION_DATE:p.published;
  const modifiedDate=report?AUG17_RESEARCH_MODIFIED_DATE:(p.sourceDate??p.published);
  const schema={"@context":"https://schema.org","@type":"Article",headline:p.title,datePublished:publicationDate,dateModified:modifiedDate,mainEntityOfPage:canonical,author:{"@type":"Organization",name:site.brand},publisher:{"@type":"Organization",name:site.brand}};
  return <><Header hidePricing/><main className="section"><article className="container guide-article"><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/><p className="eyebrow">Research · <time dateTime={publicationDate}>{['2026-09-11', '2026-09-14'].includes(publicationDate) ? 'Published: ' + formatPublicDate(publicationDate) : formatPublicDate(publicationDate)}</time>{modifiedDate!==publicationDate?<> · Updated <time dateTime={modifiedDate}>{formatPublicDate(modifiedDate)}</time></>:null}</p><h1>{p.title}</h1><p className="lead">{p.excerpt}</p><img src={p.hero??`/research-thumbnails/${p.slug}.svg`} width="1200" height="630" alt={p.title + ' research hero'} loading="eager"/>{report?<Aug17Report report={report}/>:<><h2>Key Stats</h2><div className="cards">{p.keyStats.map(s=><div className="card" key={s.label}><strong>{s.value}</strong><p>{s.label}</p></div>)}</div><div className="card"><h2>Methodology and findings</h2>{p.body.map(x=><p key={x}>{x}</p>)}{p.serviceHandoff&&<section><h2>{p.serviceHandoff.heading}</h2>{p.serviceHandoff.body.map(paragraph=><p key={paragraph}>{paragraph}</p>)}<a className="btn primary" href={p.serviceHandoff.href}>{p.serviceHandoff.label}</a></section>}<h2>Sources</h2><ol>{p.sources.map(s=><li key={s.url}><a href={s.url} rel="noreferrer">{s.name}</a> — {s.note}</li>)}</ol><h2>Related Research</h2><ul>{p.related.map(relatedSlug=><li key={relatedSlug}><a href={`/research/${relatedSlug}`}>{researchPosts.find(r=>r.slug===relatedSlug)?.title??relatedSlug}</a></li>)}</ul></div></> }</article>{!report&&<CTA/>}</main><Footer hidePricing/></>;
}
