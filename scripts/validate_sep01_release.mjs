...
const source = fs.readFileSync('app/sep01Content.ts', 'utf8');
...
assert.ok(source.includes("const published = '2026-09-01'"));
assert.ok((source.match(/September 1, 2026/g) ?? []).length >= 2, 'visible date copy missing');
assert.ok(data.includes('...sep01BlogArticles') && data.includes('...sep01ResearchArticles'));
assert.ok(sitemap.includes('...blogPosts.map') && sitemap.includes('...researchPosts.map'));
assert.ok(blogRoute.includes('mainEntityOfPage: url') && blogRoute.includes('datePublished: published') && blogRoute.includes('dateTime={published}'));
assert.ok(researchRoute.includes('mainEntityOfPage:canonical') && researchRoute.includes('datePublished:publicationDate') && researchRoute.includes('dateTime={publicationDate}'));
...
console.log('September 1 release contract passed: exactly 12 Blog + 5 Research with unique source records, routes, titles, and structured/visible dates.');
