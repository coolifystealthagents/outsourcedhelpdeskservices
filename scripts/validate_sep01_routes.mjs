...
for (const [indexPath, entries] of groups) {
  ...
  for (const {route} of entries) {
    ...
    assert.ok(html.includes('September 1, 2026'), `${route} missing visible date`);
    assert.ok(html.includes('"datePublished":"2026-09-01"'), `${route} missing structured datePublished`);
